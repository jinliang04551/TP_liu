...
{
    SearchData *data;
    int matchAllInPy;                                      //全拼音匹配
    int matchAllInWord;                                    //全汉字匹配
 ...
 */
int Tree_GetPinYin(SearchTree* tree,int aID, u2char* aText, Array* iMatchPosInPinYin);

/*
 * 获取号码匹配的号码、匹配位置
 */
int Tree_GetPhoneNum(SearchTree* tree,int aID, u2char* aText, Array* iMatchPosInPhoneNum);
...
//kmp匹配 适用于查找连续字串——号码
int IsMatchByKmp(const u2char* aText,const u2char* wordInput,Array* iMatchPosInPinYin);
...
int Word2Code(u2char aWord,WordCode *code );

int SearchCachedHit(SearchTree* tree, u2char word, Array **aHits);

/*
 * 确定某一搜索集是否匹配搜索串  return匹配的权值,<0为不匹配
 */
int IsHit(SearchTree* tree,SearchData* aData, SearchData* aSearchWordData,int iIsLogTrace);

/*
 * 确定aWordCode串的aPos位置，是否和aWord相匹配
 */
int IsMatch(SearchTree* tree,WordCode* aWordCode,int nPyCode,int nchar, unsigned int aWord );

unsigned int ChangeWordToDigit(SearchTree* tree,unsigned int Word);

int CompareWord(SearchTree* tree,unsigned int Word,unsigned int WordInput);
